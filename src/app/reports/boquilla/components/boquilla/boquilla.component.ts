...
import {BoquillaService} from '../../../../core/services/boquilla.service';

@Component({
  selector: 'app-boquilla',
  templateUrl: './boquilla.component.html',
  styleUrls: ['./boquilla.component.scss']
})
export class BoquillaComponent implements OnInit {
public titulo="Boquilla";
weatherData:any=[];//puedo crear el model para tipar la data que entra
  ...
  constructor(public elRef: ElementRef, private boquillaService:BoquillaService  ) {
    this.htmlElement = this.elRef.nativeElement;
    this.chartData = {data: [], locationName: ''};
    this.activeField = 0;
  }
  ngOnInit(): void {
    this.weatherData =this.boquillaService.getAllDataPoints();
  ...
  private buildSvg(): void {
    ...
    this.svg
      ...
      .attr("y", "20%") // set the location of the text with respect to the y-axis
      .attr("x", "40%") // set the location of the text with respect to the x-axis
      .style("fill", "#ccc") // set the font color
...
/**
  ...
  private configureXaxis(): void {
    ...
    this.x = d3Scale.scaleTime()
      .range([0, this.width])
      .domain(d3Array.extent(this.data, (d:any) => d.date));
